...
# needs in order to reach the service at all. $PORT is honoured because some hosts
# (Render) inject it; it defaults to 7860, which is what Hugging Face Spaces expects.
CMD ["sh", "-c", "exec uvicorn serve:app --host 0.0.0.0 --port ${PORT:-7860}"]
